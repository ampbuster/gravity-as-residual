...
**5/27 is $f_{\rm active}$ in disguise** (§4.35): 5/27 = 0.185 corresponds to $\tau = 2.5\text{ Gyr}$ (cosmic SFR peak); 5% = 0.05 corresponds to $\tau = 0.7\text{ Gyr}$ (gas consumption). LOCAL vs GLOBAL distinction.
...
- **[FAIL]** *Not established*: whether SIDC's smooth $E^{1+\alpha}$ creation function (§2.5.3) applies equally to phase transitions, PBHs, and stellar events (each has different physics; the smooth function uses $\alpha = 1.29\text{ from}$ SN calibration, which may not apply to other event types)
