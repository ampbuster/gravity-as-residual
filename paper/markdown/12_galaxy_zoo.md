...
- SIDC's specific quantitative predictions (the *exact* M_dyn/M_b for each galaxy) are derived from first principles. They are *qualitative* predictions calibrated to the data.
...
For each of the 11 tests, SIDC is *consistent* with the observation. But *consistency* is not *derivation*. SIDC *derives* the qualitative rule (DM tracks activity) from the dimensional projection mechanism, but it does *not derive* the specific M_dyn/M_b ratio for each galaxy from first principles.
