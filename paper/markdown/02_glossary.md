...
SIDC is a *cone* (one parent, many children, terminal at the children's level), not a *fractal* (infinite recursion). The cone has *two* transitions (4D $\to$ 3+1D, 3+1D $\to$ 2D) and *one* terminal child level. We (3+1D observers) sit *at the bottom of the cone*, just above the 2D terminal level.
...
The universe "ends" when *both* boundaries are reached: the antigravity stops *abruptly* (SIDC-specific), and matter is empty (Big Freeze). These two boundaries may happen at *similar* t\times (when the 4D event ends), but they're *distinct* in nature.
