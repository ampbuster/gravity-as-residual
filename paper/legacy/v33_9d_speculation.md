# 9D = String Theory Connection (Historical, dropped in v3.3)

> **Status**: Historical. The 9D = String Theory connection and 9D = v_Higgs match were DROPPED in v3.3 (#23 user-correction). M_Pl,4D is now derived via α-weighted GM, not via 9D scaling.

> **Superseded by**: §3.63 Equal-Universe Cascade Formula (v3.3) with M_Pl,4D = M_Pl,3D^α × M_Pl,2D^(1-α) = 4×10²³ GeV

> **Why kept**: Documents the v3.1 era exploration of 9D extension, which was empirically rejected.
...
|-----------|--------|-------|
| SIDC v3.3 | 2D, 3+1D, 4D, 5D-8D | $M_{\rm Pl,4}$ = 4×10²³ GeV |
...
$M_{\rm Pl,4}$ = 4×10²³ GeV (SIDC v3.3 §3.67k)
...
```
   ...
   4D: $M_{\rm Pl,4}$ = 4×10²³ GeV ──┤  5D: 4×10²³/α GeV (free)
...
```
                ...
                $M_{\rm string}$ = $v_{\rm Higgs}$ = 246 GeV
                          ...
                          │
   SIDC CASCADE ──────────┤
   2D: $M_{\rm Pl,2D}$ ~ 3 TeV    │   5D: 1.69×10²⁴ GeV
   3+1D: $M_{\rm Pl,3}$ = 10¹⁹ GeV│   6D: 534 GeV
   4D: $M_{\rm Pl,4}$ = 4×10²³ GeV ──┤  7D: free (not derived)
                          │   8D: 321 GeV
...
- **M_Pl,N**: M_Pl,3D = 1.22×10¹⁹ GeV (3D, our universe) vs M_Pl,4D = 4×10²³ GeV (4D bulk) vs M_Pl,2D = 3 TeV (2D universes, from L41 v3.0.22 closed). Three DIFFERENT M_Pl at three different levels. (Note: the M_Pl,2D value is from L41 holographic 2D brane, NOT 10³⁸ GeV (v3.1.2) — that value was a typo/placeholder and has been corrected.)
...
**Why M_Pl,4D = 4×10²³ GeV (Scenario X adopted)**:

In standard brane-world physics, the bulk Planck can be TeV-scale (ADD) or near the standard Planck (RS-I/II with warping). The cascade adopts M_Pl,4D = 4×10²³ GeV (4D BULK Planck) as the cascade's INFERRED value. This is consistent with:
- The cascade's M_Pl,4 ≥ 4×10²³ GeV floor (from previous observational analysis)
...
| 4D bulk (parent) | 4×10²³ GeV | DERIVED (α-weighted GM) |
