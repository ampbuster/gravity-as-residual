...
| 100 (NEW v3.0.21, REVISED v3.2, USER-CRITIQUED FOUR TIMES) | $F_p$(z) framework OVERSTATED; 'primordial = 99.93%' WRONG; cumulative gives 3.4× Ω_DM but RATIO OVER COSMIC TIME is unaddressed; f_back cannot balance DM without breaking DE | OPEN (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py, v31_all_events_cumulative.py, v31_fback_balance.py | $F_p$(z) separates primordial (99.93%) vs cumulative (0.07%) DM. **FUNDAMENTAL REVISIONS (v3.2, user-caught FOUR TIMES)**: (a) **'Primordial = 99.93%' is WRONG** (1st catch): uniform DM cannot explain AGC/KKR variation. (b) **Cumulative is NOT just SNe** (2nd catch): including all events gives 3.4× Ω_DM, AGN dominates (10⁴× SN contribution). (c) **DM/baryon ratio over cosmic time** (3rd catch): if DM only accumulates (rate ~10⁴ M_⊙/Mpc³/yr) and doesn't decay (rate 1.8×10⁻³³ M_⊙/Mpc³/yr, 10³³× slower), then DM/baryon ratio should INCREASE rapidly. But observations show roughly CONSTANT ratio (Ω_DM/Ω_b ≈ 5.4 at z=0 and z=1100). (d) **f_back cannot balance DM** (4th catch): required f_back to balance DM accumulation = 3.0×10⁻¹⁵ /s, which is 10⁷⁰× larger than the framework's calibrated value (1.22×10⁻⁸⁵). Increasing f_back by 10⁷⁰× would make DE 10⁷⁰× too large, breaking the 14% DE match. **The framework's f_back is calibrated to DE and CANNOT also balance DM.** This is a fundamental tension in the closed-loop picture. **NET RESULT (v3.2)**: The framework's DM accounting has FOUR problems: (i) 'primordial = 99.93%' is wrong, (ii) cumulative gives 3.4× Ω_DM (close but not exact), (iii) DM/baryon ratio evolution unaddressed, (iv) f_back cannot balance DM without breaking DE. The '5/5 dwarf cases' test cumulative, but the framework doesn't explain why DM/baryon ratio is constant over cosmic time. Possible resolutions (all require new physics): (1) DM has its own decay mechanism (f_back,DM > f_back,DE), (2) DM and baryons are created together at high z, (3) accumulation rate is overestimated (AGN rate uncertain), (4) framework's f_back is wrong, (5) Two-tier f_back (DE f_back and DM f_back are different). STRENGTH: per-event physics robust, all-events integration gives ~Ω_DM. WEAKNESS: framework can't explain constant DM/baryon ratio; f_back calibrated to DE can't balance DM; 4D event is a trigger not source. Closing requires: (1) Two-tier f_back (NEW: separate DM decay rate), (2) F_p(z) REJECTED, (3) per-event mass derivation, (4) event rate calibration, (5) amplification mechanism, (6) creation-with-baryons mechanism. The framework's DM picture is INCOMPLETE. |
...
**Honest verdict:** SIDC's energy-scaling rule is the *only* rule that fits the SN data, but it's not "natural" in any obvious way. The α = 1.29 value is an accident of the single calibration. The 4D cosmological lifespan is uncertain by *orders of magnitude* ($10^{19}$ to $10^{44}$ yr depending on the true α). The M_{Pl,4} ≥ 887 GeV floor in §10.3 is **specific to α = 1.29**; other α values give different (or no) floors.

SIDC's end-of-universe picture in §10.4 is therefore **not robust to the choice of α**. The qualitative prediction (DE should evolve, SFR should decline, etc.) is robust; the quantitative prediction ($M_{\rm Pl,4D}$ floor at 887 GeV, end in 1-10 Gyr) is not.
...
- Does the $M_{\rm Pl,4D}$ ≥ 887 GeV floor survive when α is allowed to vary?
...
| Direct $M_{\rm Pl,4D}$ measurement | 2030s+ (colliders) | $M_{\rm Pl,4D}$ ≥ 887 GeV | If $M_{\rm Pl,4D}$ measured at < 887 GeV, SIDC's end-of-universe timeline is falsified; if at > 887 GeV, end-of-universe is irrelevant |
