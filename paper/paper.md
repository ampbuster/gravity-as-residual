...
| 100 (NEW v3.0.21, REVISED v3.2, USER-CRITIQUED THREE TIMES) | $F_p$(z) framework OVERSTATED; 'primordial = 99.93%' WRONG; cumulative gives 3.4× Ω_DM but RATIO OVER COSMIC TIME is unaddressed; DM/baryon ratio observed constant but framework predicts increase | OPEN (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py, v31_all_events_cumulative.py | $F_p$(z) separates primordial (99.93%) vs cumulative (0.07%) DM. **FUNDAMENTAL REVISIONS (v3.2, user-caught THREE TIMES)**: (a) **'Primordial = 99.93%' is WRONG** (1st catch): uniform DM cannot explain AGC/KKR variation. (b) **Cumulative is NOT just SNe** (2nd catch): including all events gives 3.4× Ω_DM, AGN dominates (10⁴× SN contribution). (c) **DM/baryon ratio over cosmic time** (3rd catch): if DM only accumulates (rate 8.5 M_⊙/Mpc³/yr) and doesn't decay (rate 1.8×10⁻³³ M_⊙/Mpc³/yr, 10³³× slower), then DM/baryon ratio should INCREASE over cosmic time. But observations show roughly CONSTANT ratio (Ω_DM/Ω_b ≈ 5.4 at z=0 and z=1100). The framework's continuous back-flow to 4D is too small (10⁻⁸⁵ /s × 1.4×10³⁴ yr = 5.4×10⁻⁴⁴/yr) to balance the 2D universe death accumulation. **NET RESULT (v3.2)**: The framework's DM accounting has THREE problems now: (i) 'primordial = 99.93%' is wrong, (ii) cumulative gives 3.4× Ω_DM (close but not exact), (iii) DM/baryon ratio evolution unaddressed. The '5/5 dwarf cases' test cumulative, but the framework doesn't explain why DM/baryon ratio is constant over cosmic time. Possible resolutions: (1) DM has its own decay mechanism (DM → 4D at higher rate than baryons), (2) DM and baryons are created together at high z, (3) accumulation rate is overestimated (AGN rate uncertain), (4) framework's f_back is wrong. STRENGTH: per-event physics robust, all-events integration gives ~Ω_DM. WEAKNESS: framework can't explain constant DM/baryon ratio; 4D event is a trigger not source; 'primordial = 99.93%' was wrong. Closing requires: (1) DM decay mechanism or creation-with-baryons mechanism, (2) F_p(z) REJECTED, (3) per-event mass derivation, (4) event rate calibration, (5) amplification mechanism. The framework's DM picture is INCOMPLETE. |
