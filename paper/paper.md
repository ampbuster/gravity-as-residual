...
| 100 (NEW v3.0.21, REVISED v3.2) | $F_p$(z) OLD (v2.7.52, pre-Lagrangian), AGC/KKR variation UNEXPLAINED | PARTIAL (v3.2) | v21, calculations/v31_audit_v312final.py | $F_p$(z) is the primordial fraction of DM (Hill n=2, z_half = 3, v2.7.52+). REVISED v3.2: $F_p$(z) is OLD (v2.7.52, before v3 Lagrangian era) and has NOT been updated for v3.x. The Lagrangian gives per-event creation C(E) = $E^{1+\alpha}$ and pulsed return at death, but does NOT derive $F_p$(z) z-evolution. **NEW v3.2 INSIGHT (user-caught)**: If 99.93% of DM is primordial (F_p(0) = 0.9993), then per-galaxy DM variation (AGC 114905 no DM vs KKR 25 DM-rich) CANNOT be explained by cumulative 0.07% alone. The 5/5 dwarf cases (Sun, AGC, KKR, DF2/DF4, FCC 224) test the CUMULATIVE part of DM only, not the total. STRENGTH: the 5/5 cases are self-consistent for cumulative DM. WEAKNESS: the total DM variation (primordial + cumulative) is not addressed; F_p(z) functional form is a FIT (Hill n=2, z_half=3 is calibrated to cosmic SFR peak), not derived; AGC 114905 'no DM' claim is for cumulative only, primordial should still be there (uniform 99.93%). Closing requires: (1) F_p(z) derivation from Lagrangian, (2) per-galaxy E_primordial distribution (L34), (3) N_p constraint (L144). |
