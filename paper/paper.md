...
| 100 (NEW v3.0.21, REVISED v3.2, USER-CRITIQUED) | $F_p$(z) framework is FUNDAMENTALLY BROKEN: primordial can't explain AGC/KKR, cumulative gives 0.07% of Ω_DM (7.6×10⁶× too small) | OPEN (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py | $F_p$(z) separates primordial (99.93%) vs cumulative (0.07%) DM. **FUNDAMENTAL PROBLEM (v3.2, user-caught)**: BOTH options fail to explain observations. (1) **Primordial = 99.93%**: uniform across galaxies, CANNOT explain AGC 114905 (no DM) vs KKR 25 (DM-rich) variation. (2) **Cumulative = 0.07%**: cosmic SFH integrated gives 4.5×10³ M_⊙/Mpc³, which is 7.6×10⁶× TOO SMALL for Ω_DM = 3.4×10¹⁰ M_⊙/Mpc³. (3) The 4D event mass (6×10¹¹ M_⊙) is 10¹³× smaller than universe DM, and 'many 4D events' doesn't help (each creates a separate universe). (4) **The 4D event must be a TRIGGER (like Big Bang seed), not a SOURCE** — needs an amplification mechanism (inflation/phase transition) that the framework currently LACKS. **NET RESULT (v3.2)**: The framework's DM accounting is broken. The 5/5 dwarf cases (Sun, AGC, KKR, DF2/DF4, FCC 224) test the cumulative part, but cumulative is only 0.07% of Ω_DM. The '5/5 PASS' claim is misleading — it tests 0.07% of DM, not 99.93%. STRENGTH: the per-event physics (M^α scaling, C(E) = E^(1+α), f_back formula) is robust. WEAKNESS: the DM accounting is broken; the framework's 'primordial = 99.93%' claim is inconsistent with both AGC/KKR observations AND with 4D event mass; F_p(z) is OLD (v2.7.52) and has not been updated. Closing requires: (1) Reject the 'primordial = 99.93%' claim, or (2) find amplification mechanism (inflation/phase transition) for 4D event, or (3) increase per-event DM mass by ~10⁷× (multiplicity or amplification), or (4) derive F_p(z) from Lagrangian with proper accounting. The framework's current DM picture is INCOMPLETE. |
