...
**What the model does well (data backing).** The cascade has been tested against multiple independent observations and is consistent with all of them. **8/8 test categories** (RAR, cluster g_+, dwarf phase-transition, globular cluster DM, direct detection, isolated vs cluster dwarf, AGN host DM, halo mass vs M* evolution; ~410 data points) are consistent with the cascade; **7/7 specific cases** (SPARC, Tian+ 2024, Sun, DF2/DF4, FCC 224, AGC 114905, KKR 25) are also consistent. One of the eight tests (halo mass vs M* evolution) is not discriminative vs ΛCDM; the others either discriminate or pass clean null-tests.
