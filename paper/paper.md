...
**What the model does well (data backing).** The cascade has been tested against multiple independent observations and is consistent with all of them. **11/11 test categories** (RAR, cluster g_+, dwarf phase-transition, globular cluster DM, direct detection, isolated vs cluster dwarf, AGN host DM, halo M/M* vs z, missing satellites, too-big-to-fail, dSph M_dyn, MDAR, lensing flux ratio; ~430 data points) are consistent with the cascade; **7/7 specific cases** (SPARC, Tian+ 2024, Sun, DF2/DF4, FCC 224, AGC 114905, KKR 25) are also consistent. Three of the eleven tests (halo M/M* vs z, dSph M_dyn, lensing flux ratio) are not discriminative vs ΛCDM on their own; the others either discriminate or pass clean null-tests.
