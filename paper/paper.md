...
| 100 (NEW v3.0.21, REVISED v3.2) | $F_p$(z) OLD (v2.7.52, pre-Lagrangian), AGC/KKR variation UNEXPLAINED, 4D event mass 10¹³× too small for primordial DM | PARTIAL (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py | $F_p$(z) is the primordial fraction of DM (Hill n=2, z_half = 3, v2.7.52+). REVISED v3.2: $F_p$(z) is OLD (v2.7.52, before v3 Lagrangian era) and has NOT been updated for v3.x. The Lagrangian gives per-event creation C(E) = $E^{1+\alpha}$ and pulsed return at death, but does NOT derive $F_p$(z) z-evolution. **NEW v3.2 INSIGHT (user-caught)**: (1) If 99.93% of DM is primordial (F_p(0) = 0.9993), then per-galaxy DM variation (AGC 114905 no DM vs KKR 25 DM-rich) CANNOT be explained by cumulative 0.07% alone. (2) The 4D event mass (6×10¹¹ M_⊙, Scenario X) is 10¹³× SMALLER than the universe's total DM mass (~10²⁴ M_⊙). The 4D event cannot be the source of 99.93% of DM if it's galaxy-scale. The framework's claim that 'primordial 2D universe deaths provide 99.93% of DM' is INCONSISTENT with the 4D event mass in Scenario X. The 5/5 dwarf cases (Sun, AGC, KKR, DF2/DF4, FCC 224) test the CUMULATIVE part of DM only, not the total. STRENGTH: the 5/5 cases are self-consistent for cumulative DM. WEAKNESS: (a) total DM variation (primordial + cumulative) is not addressed; (b) F_p(z) functional form is a FIT, not derived; (c) AGC 114905 'no DM' is for cumulative only, primordial should still be there; (d) 4D event mass is 10¹³× too small to be source of primordial DM. Closing requires: (1) F_p(z) derivation from Lagrangian, (2) per-galaxy E_primordial distribution (L34), (3) N_p constraint (L144), (4) reconcile 4D event mass with universe DM mass (NEW: requires either universe-scale 4D event, or amplification mechanism). |
