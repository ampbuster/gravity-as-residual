...
| 100 (NEW v3.0.21, REVISED v3.2) | $F_p$(z) OLD (v2.7.52, pre-Lagrangian), AGC/KKR variation UNEXPLAINED, 4D event mass 10¹³× too small for primordial DM, AMPLIFICATION MECHANISM MISSING | PARTIAL (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py | $F_p$(z) is the primordial fraction of DM (Hill n=2, z_half = 3, v2.7.52+). REVISED v3.2: $F_p$(z) is OLD (v2.7.52, before v3 Lagrangian era) and has NOT been updated for v3.x. **NEW v3.2 INSIGHTS (user-caught)**: (1) If 99.93% of DM is primordial (F_p(0) = 0.9993), then per-galaxy DM variation (AGC 114905 no DM vs KKR 25 DM-rich) CANNOT be explained by cumulative 0.07% alone. (2) The 4D event mass (6×10¹¹ M_⊙, Scenario X) is 10¹³× SMALLER than the universe's total DM mass (~10²⁴ M_⊙). (3) **'Many 4D events' does NOT resolve the mass gap** (user-caught): each 4D event creates a separate causally-disconnected universe, so multiple 4D events give multiple universes, not more DM per universe. (4) **The 4D event must be a TRIGGER (like Big Bang seed), not a SOURCE**: standard cosmology has Big Bang seed (~10²⁰ J) → inflation → universe (10⁶² J), amplification factor 10⁴². The framework's 4D event (10⁵⁹ J) → universe (10²⁴ M_⊙) requires amplification factor 10¹³. **The framework lacks an explicit amplification mechanism** (inflation, phase transition, vacuum decay) to bridge this gap. The 5/5 dwarf cases test the CUMULATIVE part of DM only, not the total. STRENGTH: the 5/5 cases are self-consistent for cumulative DM. WEAKNESS: (a) total DM variation not addressed; (b) F_p(z) is FIT, not derived; (c) AGC 114905 'no DM' is for cumulative only; (d) 4D event mass is 10¹³× too small for primordial DM; (e) AMPLIFICATION MECHANISM MISSING. Closing requires: (1) F_p(z) derivation from Lagrangian, (2) per-galaxy E_primordial distribution (L34), (3) N_p constraint (L144), (4) AMPLIFICATION MECHANISM (NEW: inflation/phase transition/vacuum decay), (5) reconcile 4D event mass with universe DM mass. |
