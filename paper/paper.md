...
| 100 (NEW v3.0.21, REVISED v3.2, USER-CRITIQUED TWICE) | $F_p$(z) framework OVERSTATED: including all events (not just SNe) gives 3.4× of Ω_DM, so 'primordial = 99.93%' was WRONG; DM is mostly CUMULATIVE | PARTIAL → OPEN (v3.2) | v21, calculations/v31_audit_v312final.py, v31_scenario_X.py, v31_all_events_cumulative.py | $F_p$(z) separates primordial (99.93%) vs cumulative (0.07%) DM. **FUNDAMENTAL REVISION (v3.2, user-caught TWICE)**: (a) **'Primordial = 99.93%' is WRONG** (user-caught): uniform DM cannot explain AGC 114905 (no DM) vs KKR 25 (DM-rich) variation. (b) **Cumulative is NOT just SNe** (user-caught): the framework says EVERY energetic event creates 2D universes. Including all events (SNe, AGN, GRB, TDE, magnetar, solar flare, quasar): TOTAL = 1.17×10¹¹ M_⊙/Mpc³, which is 3.4× Ω_DM = 3.4×10¹⁰ M_⊙/Mpc³. (c) **AGN dominates** (5.6×10¹⁰ M_⊙/Mpc³, 10⁴× the SN contribution) because AGN per-event mass (5.6×10⁷ M_⊙) is 10¹¹× larger than SN. (d) The 4D event mass (6×10¹¹ M_⊙) is 10¹³× smaller than universe DM, and 'many 4D events' doesn't help. (e) The 4D event is a TRIGGER, not a SOURCE — needs amplification mechanism. **NEW v3.2 PICTURE**: DM is mostly CUMULATIVE (from current 3D events), with AGN being the dominant contributor. The '5/5 dwarf cases' still test cumulative, but cumulative is ~3.4× Ω_DM (not 0.07%). The 3.4× discrepancy could be resolved by: (i) tuning event rates (AGN rate uncertain), (ii) multiplicity factor per event, (iii) per-event mass not exactly E/c². STRENGTH: per-event physics (M^α scaling, C(E) = E^(1+α), f_back) is robust; all-events integration gives ~Ω_DM (within 3.4×). WEAKNESS: F_p(z) is OLD (v2.7.52) and 'primordial = 99.93%' is WRONG; 4D event needs amplification; per-event mass might not be exactly E/c². Closing requires: (1) F_p(z) REJECTED as the wrong decomposition — DM is mostly CUMULATIVE; (2) per-event mass derivation from Lagrangian (not just E/c²); (3) event rate calibration (especially AGN); (4) amplification mechanism for 4D event (if primordial still plays a role). |
