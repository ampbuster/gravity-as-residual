**Version 2.5** (June 2026) — *cascade matches ΛCDM at all z.* Major milestone: with the broader principle (Thomson scattering dominates R(z) at z > 4), the cascade's r(z) ≈ (1+z)³ for all z. **Limitation 31 CLOSED**. Three smoking guns: AGC/KKR bifurcation, scale-time invariance, cascade matches ΛCDM structure at all z. The cascade is qualitatively consistent with H_0 = 70 ± 3 across all measurements but does not derive a specific H_0 value (see §2.6.1 honest framework). Honest H_0 framework added in v2.5 commit 281; overstatement audit (5 claims cleaned) in commit 282.
