...
- The 50-orders-of-magnitude tension between 2D-frame and 3+1D-frame masses

**The required $e^{-ky} \sim 10^{-48}$ corresponds to 2D universes at bulk depth $y \sim 100$ AdS_5 radii.** This is deep but not unreasonable. A full Boltzmann code (CAMB-based, see `tempcalc/cascade_camb_time_compressed.py`) shows that the time compression has consistent effects on the H(z) calculation.
