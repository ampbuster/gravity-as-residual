#!/usr/bin/env python3
"""
Test more SFR-dependent parameter forms.

The f_active ∝ log(SFR) form gave 0.263 median residual.
Let me try other forms to see if we can do better.
"""
...
# Baseline
cases['Single'] = (lambda M, k, s: 0.05, lambda M, k, s: 0.25, lambda M, k, s: 0.15)

# f_active ∝ log(SFR) - the breakthrough
cases['f_active = 0.02 + 0.01 log(SFR)'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.02 + 0.01 * math.log10(s + 1))),
    lambda M, k, s: 0.25,
    lambda M, k, s: 0.15
)

# Different coefficient
cases['f_active = 0.05 + 0.02 log(SFR)'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.05 + 0.02 * math.log10(s + 1))),
    lambda M, k, s: 0.25,
    lambda M, k, s: 0.15
)

# f_active ∝ log(SFR/M_disk) = log(sSFR)
cases['f_active = 0.05 + 0.01 log(sSFR)'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.05 + 0.01 * (math.log10(s + 1e-10) - math.log10(M) - 0.5))) if M > 0 else 0.05,
    lambda M, k, s: 0.25,
    lambda M, k, s: 0.15
)

# f_active = SFR / (SFR + SFR_sat) with saturation
# At low SFR, f_active ~ SFR/SFR_sat
# At high SFR, f_active saturates
SFR_sat = 1.0
cases['f_active = SFR/(SFR+1) (saturating)'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.1 * s / (s + SFR_sat))),
    lambda M, k, s: 0.25,
    lambda M, k, s: 0.15
)

# f_active ∝ (1 + log(SFR))
cases['f_active = 0.01 (1 + 0.5 log(SFR))'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.01 * (1 + 0.5 * math.log10(s + 1)))),
    lambda M, k, s: 0.25,
    lambda M, k, s: 0.15
)

# r_core_frac also depends on SFR
# Higher SFR = more concentrated recent activity = smaller core
cases['f_active(SFR) + r_core ∝ 1/SFR'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.02 + 0.01 * math.log10(s + 1))),
    lambda M, k, s: max(0.05, min(0.5, 0.3 / math.log10(s + 10))),
    lambda M, k, s: 0.15
)

# scale ∝ f_active (both track SFR)
cases['f_active(SFR), scale = 3 * f_active'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.02 + 0.01 * math.log10(s + 1))),
    lambda M, k, s: 0.25,
    lambda M, k, s: max(0.05, min(0.5, 3 * (0.02 + 0.01 * math.log10(s + 1))))
)

# Best combined
cases['All 3 ∝ log(SFR)'] = (
    lambda M, k, s: max(0.001, min(0.3, 0.02 + 0.01 * math.log10(s + 1))),
    lambda M, k, s: max(0.1, min(0.5, 0.2 + 0.02 * math.log10(s + 1))),
    lambda M, k, s: max(0.05, min(0.5, 0.1 + 0.05 * math.log10(s + 1)))
)
